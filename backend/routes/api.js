...
router.get("/road-info", auth, async (req, res) => {
  ...
  `;
  try {
    const resp = await axios.post(overpass, q, { headers: { "Content-Type": "text/plain" }, timeout: 15000 });
    const elems = resp.data.elements || [];
    if (!elems.length) return res.json({ found: false, message: "No highway nearby" });
    ...
    const speeds = limits.map(item => {
      const vals = [item.expressway, item.fourLane, item.municipal, item.otherRoads];
      return { vehicleClass: item.vehicleClass, maxSpeed: vals[col] };
    });
    res.json({ found: true, highwayTag: highway, roadType, columnIndex: col, speeds, rawTags: tags });
  } catch (err) {
...
router.post("/checkSpeed", auth, async (req, res) => {
  try {
    const { vehicleClass, roadType, currentSpeed } = req.body;
    if (!vehicleClass || !roadType || currentSpeed == null) return res.status(400).json({ error: "vehicleClass, roadType, currentSpeed required" });
    const limits = await SpeedLimit.findOne({ vehicleClass });
    if (!limits) return res.status(404).json({ error: "No speed limit configured for this vehicle class" });
    // decide column
    let col = "otherRoads";
    const t = (roadType+"").toLowerCase();
    if (t.includes("expressway") || t.includes("motorway")) col = "expressway";
    else if (t.includes("state highway") || t.includes("national") || t.includes("4 lane")) col = "fourLane";
    else if (t.includes("municipal") || t.includes("city") || t.includes("town")) col = "municipal";
    const limit = limits[col];
    if (limit == null) return res.json({ status: "NoLimit", message: "No limit set for this vehicle on this road type" });
    if (Number(currentSpeed) <= Number(limit)) return res.json({ status: "OK", message: `Within limit (${limit} km/h)` });
    return res.json({ status: "Overspeed", message: `Limit ${limit} km/h, driven ${currentSpeed} km/h` });
  } catch (err) {
    console.error(err);
